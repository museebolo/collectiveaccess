...
initdb()
{
	...
	mysql -h ${MYSQL_ADDR} -uroot -p${MYSQL_ROOT_PASS} \
	      -e "CREATE DATABASE IF NOT EXISTS ${CA_DB_NAME};"
	mysql -h ${MYSQL_ADDR} -uroot -p${MYSQL_ROOT_PASS} \
	      -e "GRANT ALL PRIVILEGES ON ${CA_DB_NAME}.* TO '${CA_DB_USER}' IDENTIFIED BY '${CA_DB_PASS}';"
}
